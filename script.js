...
function preloadFeaturedIframes() {
    ...
    if (!autoplayEnabled) return Promise.resolve();
...
// ── ADMIN SESSION (REAL SUPABASE VALIDATION) ──
// Mengecek sesi asli dari server Supabase
...
// Melindungi fungsi yang merubah data
async function requireAuth() {
    const isActive = await checkAdminSession();
    if (!isActive) {
        toast('Sesi tidak valid atau telah berakhir. Silakan login kembali.', 'error');
        closeAdminPanel();
        throw new Error('Unauthorized');
    }
}

// Pengecekan berkala panel terbuka
setInterval(async () => {
...
function ytExtract(s) { if (!s || typeof s !== 'string') return null; const m = s.match(/(?:youtube\.com\/watch\?v=|youtu\.be\/|youtube\.com\/embed\/)([a-zA-Z0-9_-]{11})/) || s.match(/^([a-zA-Z0-9_-]{11})$/); return m ? m[1] : null; }
let _tt;
...
async function togglePresetTag(inputId, tag, btn) { 
    await requireAuth(); 
    let tags = getTagsArray(inputId); 
    if (tags.includes(tag)) { tags = tags.filter(t => t !== tag); btn.classList.remove('active'); } 
...
function injectPreloadedFeatured() {
    ...
    featuredCardEls.forEach(card => {
        ...
        if (preloaded) {
            ...
            featuredPreloadMap.delete(ytId);
        } else {
...
<button class="inline-cancel-btn" onclick="toggleEdit('${c.id}')">Cancel</button>
    ...
    sortableInstance = Sortable.create(listEl, { handle: '.drag-handle', animation: 180, ghostClass: 'sortable-ghost', chosenClass: 'sortable-chosen', onEnd: async (evt) => { if (evt.oldIndex === evt.newIndex) return; const moved = cards.splice(evt.oldIndex, 1); cards.splice(evt.newIndex, 0, moved[0]); let note = el('sort-saving'); if (!note) { note = document.createElement('div'); note.id = 'sort-saving'; note.style.cssText = 'text-align:center;padding:8px;font-family:"Space Mono",monospace;font-size:11px;color:var(--accent)'; listEl.insertAdjacentElement('afterend', note); } note.textContent = '⟳ Saving order...'; await Promise.all(cards.map((c, i) => sb.from('mv_works').update({ sort_order: i }).eq('id', c.id))); note.remove(); toast('Order saved! ✓', 'success'); renderGrid(true); } });
}
...
function initGridSortable() {
    ...
    gridSortable = Sortable.create(grid, {
        ...
        onEnd: (evt) => {
            ...
            cards.splice(evt.newIndex, 0, moved[0]);
...
async function init() {
    ...
    setTimeout(() => {
        ...
        setTimeout(async () => {
            ...
            // Cek langsung sesi asli saat pertama buka
            const isActive = await checkAdminSession();
            if (isActive) el('admin-panel').classList.add('open');
